...
#include "TeamCivitasPage.h"
#include "Version.h"
#include "modplatform/ModIndex.h"
#include "modplatform/modrinth/ModrinthAPI.h"
#include "ui/dialogs/CustomMessageBox.h"
#include "ui_TeamCivitasPage.h"

#include "TeamCivitasModel.h"
...
TeamCivitasPage::TeamCivitasPage(NewInstanceDialog* dialog, QWidget* parent)
    : QWidget(parent), m_ui(new Ui::TeamCivitasPage), m_dialog(dialog), m_fetch_progress(this, false)
{
    m_ui->setupUi(this);
    createFilterWidget();

    m_ui->searchEdit->installEventFilter(this);
    m_model = new TeamCivitas::ModpackListModel(this);
    ...
    connect(&m_search_timer, &QTimer::timeout, this, &TeamCivitasPage::triggerSearch);
    ...
    connect(m_ui->sortByBox, &QComboBox::currentIndexChanged, this, &TeamCivitasPage::triggerSearch);
    connect(m_ui->packView->selectionModel(), &QItemSelectionModel::currentChanged, this, &TeamCivitasPage::onSelectionChanged);
    connect(m_ui->versionSelectionBox, &QComboBox::currentIndexChanged, this, &TeamCivitasPage::onVersionSelectionChanged);

    m_ui->packView->setItemDelegate(new ProjectItemDelegate(this));
    m_ui->packDescription->setMetaEntry(metaEntryBase());
}

TeamCivitasPage::~TeamCivitasPage()
{
    delete m_ui;
}

void TeamCivitasPage::retranslate()
{
    m_ui->retranslateUi(this);
}

void TeamCivitasPage::openedImpl()
{
    BasePage::openedImpl();
    suggestCurrent();
    triggerSearch();
...
bool TeamCivitasPage::eventFilter(QObject* watched, QEvent* event)
{
...
void TeamCivitasPage::onSelectionChanged(QModelIndex curr, [[maybe_unused]] QModelIndex prev)
{
    ...
    if (!m_current->extraDataLoaded) {
        qDebug() << "Loading TeamCivitas modpack information";
    ...
    if (!m_current->versionsLoaded || m_filterWidget->changed()) {
        qDebug() << "Loading TeamCivitas modpack versions";
...
void TeamCivitasPage::suggestCurrent()
{
    if (!isOpened) {
        return;
    }
    ...
    for (auto& ver : m_current->versions) {
        if (ver.addonId == m_selectedVersion) {
            ...
            QString editedLogoName = "TeamCivitas_" + m_current->logoName;
...
void TeamCivitasPage::triggerSearch()
{
...
void TeamCivitasPage::onVersionSelectionChanged(int index)
{
    if (index == -1) {
        m_selectedVersion = "";
        return;
...
void TeamCivitasPage::setSearchTerm(QString term)
{
    m_ui->searchEdit->setText(term);
}

QString TeamCivitasPage::getSerachTerm() const
{
    return m_ui->searchEdit->text();
}

void TeamCivitasPage::createFilterWidget()
{
    ...
    connect(m_filterWidget.get(), &ModFilterWidget::filterChanged, this, &TeamCivitasPage::triggerSearch);
